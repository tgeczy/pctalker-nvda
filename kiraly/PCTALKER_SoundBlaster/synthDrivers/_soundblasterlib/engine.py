...
def olvas_intonation_speeds(codes: list[int],
    ...
    Original ASM behavior:
      ...
      - ',' starts/continues veszkov, adding 4 each phoneme
    ...
    for i in range(len(codes)):
        ...
        # Sound Blaster 5.01 active comma ramp: +4, reset above 12.
...
# ---------------------------------------------------------------------------
# PCTALKER Sound Blaster 5.01 recursive echo
# ---------------------------------------------------------------------------

def apply_soundblaster_echo(samples: bytes, delay_samples: int = 250) -> bytes:
    """
    Reconstruct the active echo loop in OLVAS_S.ASM HANG.

    The original code uses a circular delay buffer and feeds the delayed
    OUTPUT back at approximately 50% amplitude:

        contribution ~= (delayed - 128) / 2
        output = saturate(current + contribution)

    Because the output is written back into the delay line, repeats decay
    recursively.

    Modern convenience: delay_samples <= 0 disables echo. The original ASM
    default is 250 samples. At 8500 Hz this is about 29.4 ms.
    """
    if not samples or delay_samples <= 0:
        return samples

    delay_samples = max(1, min(4999, int(delay_samples)))
    ring_len = delay_samples + 1

    # OLVAS_S.ASM declares hangbuf as 127-filled storage.
    ring = bytearray([127]) * ring_len
    head = delay_samples
    tail = 0
    ...
    for i, current in enumerate(samples):
        ...
        # Exact integer-style feedback used by the ASM:
        # SHR AH,1 ; SUB AH,64
        contribution = (delayed >> 1) - 64
...
class OlvasEngine:
    ...
    def __init__(self, base_dir: Path | str | None = None,
        ...
        self.asm = self.data_dir / "OLVAS_S.ASM"
        ...
        self.echo_delay = max(0, min(4999, int(echo_delay)))
    ...
    def synthesize_segments(self, text: str):
        ...
        segments = synthesize_mixed_segments(
            text,
            self.asm,
            self.szotar,
            self.raw,
        ...
        return [
            (
                kind,
                apply_soundblaster_echo(pcm, self.echo_delay)
                if kind == "speech" else pcm,
            )
