...
# Experimental CWI-2 / CP-HU mapping for Hungarian text.
CWI2_ENCODE = {
    "ü":0x81, "é":0x82, "Í":0x8D, "Á":0x8F, "É":0x90,
    "ő":0x93, "ö":0x94, "Ó":0x95, "ű":0x96, "Ú":0x97,
    "Ű":0x98, "Ö":0x99, "Ü":0x9A, "á":0xA0, "í":0xA1,
    "ó":0xA2, "ú":0xA3, "Ő":0xA7,
}
CWI2_DECODE = {v:k for k,v in CWI2_ENCODE.items()}
...
def cwi2_decode(data: bytes, errors: str = "replace") -> str:
    out=[]
    for b in data:
        if b < 0x80: out.append(chr(b))
        elif b in CWI2_DECODE: out.append(CWI2_DECODE[b])
        elif errors == "ignore": continue
        else: out.append(chr(b))
    return "".join(out)

def encode_dos(text: str, errors: str = "replace") -> bytes:
    """
    Encode text for the original PCTALKER speaker tables.

    OLVASSP.ASM contains silent placeholder entries for the genuine CWI-2
    uppercase codes Ó=0x95, Ő=0xA7 and Ű=0x98, while the lowercase codes
    ó=0xA2, ő=0x93 and ű=0x96 point to the real recorded speech elements.

    Preserve canonical CWI-2 in cwi2_encode(), but for PCTALKER table lookup
    map only these three uppercase letters to their lowercase equivalents.
    """
    text = text.replace("Ó", "ó").replace("Ő", "ő").replace("Ű", "ű")
...
# ---------------------------------------------------------------------------
# Extended English pronunciation dictionary
# ---------------------------------------------------------------------------

ENGLISH_WORD_RE = re.compile(r"[A-Za-z]+(?:'[A-Za-z]+)?")

@lru_cache(maxsize=4)
def load_extended_english_lexicon(path: str):
    lexicon = {}
    p = Path(path)
    if not p.is_file():
        return lexicon
...
def apply_extended_english(text: str, asm_path: Path) -> str:
    lexicon_path = asm_path.parent / "english_extended.tsv"
    lexicon = load_extended_english_lexicon(str(lexicon_path))
    if not lexicon:
        return text

    def repl(match):
        word = match.group(0)
        return lexicon.get(word.casefold(), word)

    return ENGLISH_WORD_RE.sub(repl, text)
...
def expand_numbers(text: str) -> str:
    """
    Expand Hungarian cardinal and decimal numbers.
    ...
    def repl(m: re.Match) -> str:
        ...
        spoken = prefix + number_to_hungarian(int(whole))
...
def synthesize_text_segment(text: str, asm: Path, szotar: Path, raw: bytes, table,
                            intonation: bool = True, show_intonation: bool = False,
                            language_mode: str = "hu_HU") -> bytes:
    if language_mode == "en_US":
        text = apply_extended_english(text, asm)
        expanded = expand_numbers(text)
        encoded = expanded.replace("ű", "ü").replace("Ű", "Ü").replace("Ó", "ó").encode(
            DOS_ENCODING, errors="replace"
        )
        phoneme_bytes = convert_bytes(encoded, asm)
    else:
        expanded = expand_numbers(text)
        _, phoneme_bytes = dictionary_convert(expanded, asm, szotar)
...
def synthesize_mixed_text(text: str, asm: Path, szotar: Path,
    ...
    parts = synthesize_mixed_segments(
        ...
        show_audio, intonation, show_intonation, language_mode
    )
...
class OlvasEngine:
    ...
    def __init__(self, base_dir: Path | str | None = None,
        ...
        self.language_mode = language_mode if language_mode in ("hu_HU", "en_US") else "hu_HU"
    ...
    def speak(self, text: str, interrupt: bool = True):
        ...
        User speed applies only to synthesized speech.
        Embedded WAV/MP3 segments always play at normal SAMPLE_RATE.

        interrupt=True (default) matches screen-reader behavior.
        """
        if self._closed:
            raise RuntimeError("OLVAS engine is closed.")
        ...
        with self._speak_lock:
            ...
            for kind, samples in self.synthesize_segments(text):
                rate = (
...
def main() -> int:
    ...
    ap.add_argument("--language", choices=("hu", "en"), default="hu",
                    help="Text preprocessing language: hu=original SZOTAR.TBL, en=English UTF-8 lexicon.")
    args = ap.parse_args()
    try:
    ...
    if args.engine_api and (args.text is not None or args.stdin):
        ...
        if args.no_play or args.wav:
            ...
            with OlvasEngine(
                ...
                speed=args.speed,
                language_mode=("hu_HU" if args.language == "hu" else "en_US"),
            ) as eng:
                eng.speak(api_text)
                eng.wait()
            return 0
    ...
    if args.phoneme is not None:
        ...
        mixed_samples = synthesize_mixed_text(
            ...
            not args.no_intonation, args.show_intonation, language_mode
        )
        if args.show_conversion:
            print("Dictionary result:", normalized.decode(DOS_ENCODING, errors="replace"))
